  ...
  return (
    <PopoverPrimitive.Trigger ref={forwardedRef as any} asChild>
      <div
        ...
        {...props}
        className={ny(
          "border-border-input ring-offset-bgc focus:ring-etc-ring flex size-full min-h-10 items-center justify-between whitespace-nowrap rounded-md border bg-transparent px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-1 [&>span]:line-clamp-1",
...
>(
  (
    ...
    if (!value.length || !firstRendered) {
      return (
        <span className="text-fgc-muted pointer-events-none">
          {placeholder}
        </span>
      )
    }
        ...
        >
          {renderItems.map((value) => {
            ...
            const el = (
              ...
              >
                <span>{child}</span>
                <X className="text-fgc-muted group-hover/multi-select-badge:text-fgc ml-1 size-3" />
              </Badge>
            )
          ...
          {renderRemain ? (
            <span className="text-fgc-muted py-.5 text-xs leading-4">
              +{renderRemain}
            </span>
          ) : null}
        </div>
  ...
  (
    ...
    return (
      <CommandItem
        ...
        value={value}
        className={ny(
          disabled && "text-fgc-muted cursor-not-allowed",
          className
        )}
        disabled={disabled}
